...
# Ensure other code remains the same, only update the responsive section.
...
def create_layout(user_data):
    ...
    # Split for banner + body + toolbox
    layout.split(
        Layout(name="header", size=11),  # Banner
        Layout(name="main_content", ratio=1),  # Will contain info boxes
        Layout(name="toolbox_area", size=28)  # Fixed size for toolbox
    )

    # Split main_content for the info boxes
    layout["main_content"].split_row(
        Layout(name="left", ratio=1),  # User info
        Layout(name="right", ratio=1)  # Tool credentials
    )
...
"""
    ...
    layout["left"].update(
        Panel(user_info, title="🌐 User Info", border_style="bold cyan", padding=(0, 0))
    )
...
"""
    layout["right"].update(
        Panel(tool_info, title="🌊 Tool Credentials", border_style="bold cyan", padding=(0, 0))
    )

    # Toolbox content - now in its own dedicated area below
    toolbox_content = f"""
...
"""
    ...
    layout["toolbox_area"].update(
        Panel(
            Align.center(toolbox_content),
            title="🌐 Toolbox",
